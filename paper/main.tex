...
thresholds. See Table~\ref{tab:hypotheses} for details.}
...
\textbf{Feature 592} (the intestinal crypt ``master switch'' described
above) is similarly graph-suppressed ($\text{dep} = -0.50$): despite
being essential for colonic clustering (100\% prototype reassignment
when ablated), the spatial graph modulates its activation downward.
This suggests the model uses neighborhood context to gate crypt-specific
signals.
...
Applying the circuit tracing protocol described in Section~2.5, we
discover directed causal relationships between SAE features across
Novae's GATv2 layers. This reveals how spatial concepts are composed
and regulated across the model's depth.
